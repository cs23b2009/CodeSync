    ...
    try {
        ...
        try {
          ...
          `,
                ...
                next: { revalidate: 3600 } // Cache past contests for 1 hour
            });

            if (pastResponse.ok) {
                const pastData = await pastResponse.json();
                const rawPastContests = pastData.data?.pastContests?.contests || [];
                // Only take the most recent 10 past contests to keep payload small
                pastContests = rawPastContests.slice(0, 10);
            } else {
